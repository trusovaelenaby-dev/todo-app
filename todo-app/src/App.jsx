...
const SAMPLES = [
  { id:"s1", text:"Утренняя зарядка 🏃", category:"personal", type:"permanent", createdAt: todayKey() },
  { id:"s2", text:"Выпить 8 стаканов воды 💧", category:"personal", type:"permanent", createdAt: todayKey() },
  { id:"s3", text:"Проверить рабочую почту", category:"work", type:"permanent", createdAt: todayKey() },
  { id:"s4", text:"Написать квартальный отчёт", category:"work", type:"temporary", createdAt: todayKey(), done:false, doneAt:null },
  { id:"s5", text:"Купить подарок другу 🎁", category:"personal", type:"temporary", createdAt: todayKey(), done:false, doneAt:null },
];
...
function fmtWeekday(s) {
  return new Date(s+"T12:00:00").toLocaleDateString("ru-RU",{weekday:"long",day:"numeric",month:"long"});
}
...
function ConfettiLayer({ trigger }) {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      pRef.current = pRef.current
        .map(p => ({ ...p, x:p.x+p.vx, y:p.y+p.vy, vy:p.vy+0.45, vx:p.vx*0.98, rot:p.rot+p.rs, alpha:p.alpha-0.016 }))
        .filter(p => p.alpha > 0);
      pRef.current.forEach(p => {
        ctx.save(); ctx.globalAlpha = Math.max(0, p.alpha);
        ctx.fillStyle = p.col; ctx.translate(p.x, p.y); ctx.rotate(p.rot * Math.PI/180);
  ...
  useEffect(() => {
    if (!trigger) return;
    const { x, y } = trigger;
    const burst = Array.from({length:65}, (_,i) => ({
      id: Date.now()+i, x, y,
      vx: (Math.random()-.5)*16, vy: -(Math.random()*13+3),
      col: CONFETTI_COLS[i % CONFETTI_COLS.length],
      sz: Math.random()*10+4, rot: Math.random()*360,
      rs: (Math.random()-.5)*22, alpha: 1,
      shape: Math.random()>.45 ? "r":"e",
    }));
    pRef.current = [...pRef.current, ...burst];
  }, [trigger]);
...
function StatsView({ tasks, completions }) {
  ...
  const summary = (task) => {
    if (task.type==="temporary") {
      return task.done
        ? { txt:`✓ ${fmtDay(task.doneAt)}`, pct:100, color:"#2e6e3a" }
        : { txt:"В процессе", pct:null, color:"#c07b5a" };
    }
    const valid = dates.filter(d => d >= task.createdAt);
  ...
  const totalSlots = permTasks.reduce((a,t)=>a+dates.filter(d=>d>=t.createdAt).length,0);
  const doneSlots  = permTasks.reduce((a,t)=>a+dates.filter(d=>d>=t.createdAt&&completions[d]?.[t.id]).length,0);
  ...
  return (
    <div>
        ...
        <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
          ...
          {[7,14,30].map(n=>(
            <button key={n} className="sb" style={{fontSize:11,padding:"4px 10px"}}
              onClick={()=>{setFrom(daysAgo(n-1));setTo(todayKey());}}>
              {n}д
            </button>
          ))}
        </div>
      ...
      {overallPct !== null && (
        <div style={S.banner}>
          <div style={{fontSize:12,color:"#5a4027",marginBottom:4}}>Общий % выполнения (постоянные задачи)</div>
          <div style={{fontSize:28,fontFamily:"'Playfair Display',serif",fontWeight:700,
            color:overallPct>=70?"#2e6e3a":overallPct>=40?"#9c7c00":"#8b3a1a"}}>
            {overallPct}%
          </div>
            ...
            <table style={S.stbl}>
                ...
                <tr style={{borderBottom:"1px solid #e8ddc8"}}>
                  ...
                  {dates.map(d=>(
                    <th key={d} style={{...S.sth,width:28,textAlign:"center",padding:"8px 3px 2px",
                      color:d===todayKey()?"#c07b5a":"#b8a88a",
                      fontWeight:d===todayKey()?700:400, fontSize:10}}>
              ...
              <tbody>
                {fTasks.map((task,i)=>{
                  ...
                  const cc  = CAT_COLORS[task.category]||CAT_COLORS.other;
                  return (
                    <tr key={task.id} style={{borderTop:i>0?"1px dashed #f0e8d8":"none",
                      background:i%2===0?"#fffdf7":"#fffbf4"}}>
                      <td style={{...S.stnm,padding:"9px 8px 9px 10px"}}>
                        <div style={{fontSize:12,color:"#2d1f0f",lineHeight:1.4}}>{task.text}</div>
                        <div style={{display:"flex",gap:4,marginTop:3}}>
                          <span style={{fontSize:9,background:cc.bg,color:cc.text,padding:"1px 6px",borderRadius:10}}>
                            {CATS[task.category]}
                          </span>
                      ...
                      <td style={{padding:"8px 10px 8px 4px",textAlign:"center",minWidth:90}}>
                        {sum.pct!==null ? (
                          <div>
                            <div style={{fontSize:15,fontWeight:700,color:sum.color,
                              fontFamily:"'Playfair Display',serif"}}>{sum.txt}</div>
      ...
      <div style={{fontSize:10,color:"#c4a97d",marginTop:10,textAlign:"right"}}>
        ✓ выполнено · · не выполнено · — до создания задачи
      </div>
    </div>
  );
}
...
function Empty({ text }) {
  return <div style={{padding:"18px 16px",textAlign:"center",fontSize:12,color:"#b8a88a",fontStyle:"italic"}}>{text}</div>;
}

// ─── Main App ─────────────────────────────────────────────────────────────────
export default function App() {
  const [tasks, setTasks]         = useState(() => load("tdtasks-v3", SAMPLES));
  ...
  useEffect(() => { save("tdcompl-v3", completions); }, [completions]);
  ...
  const fire = useCallback((e) => {
    setConfetti({ x: e?.clientX ?? window.innerWidth/2, y: e?.clientY ?? window.innerHeight/2, ts: Date.now() });
  }, []);
  ...
  return (
    <div style={S.root}>
      ...
      <div style={S.paper}>
        ...
        <div style={S.tabs}>
          {[["today","📋 Сегодня"],["stats","📊 Статистика"]].map(([id,lbl])=>(
          ...
          {view==="today" ? (
            <>
              ...
              <Section icon="🔁" title="Постоянные"
                subtitle="Появляются каждый день, выполнение сбрасывается в полночь"
...
.tr {
  ...
  background:#fffdf7; border-bottom:1px dashed #d9cdb8;
...
.chk {
  width:22px; height:22px; border:2px solid #c4a97d; border-radius:5px;
  flex-shrink:0; display:flex; align-items:center; justify-content:center;
  font-size:13px; transition:background .2s, border-color .2s, transform .15s;
}
...
.fab {
  ...
  border:none; font-size:26px; cursor:pointer; display:flex; align-items:center;
  justify-content:center; transition:background .2s, transform .2s; line-height:1; flex-shrink:0;
}
.fab:hover { background:#5a3d28; transform:scale(1.1) rotate(90deg); }
.fab:active { transform:scale(.95); }

.tabbt {
  font-family:'IBM Plex Mono',monospace; font-size:13px; padding:11px 24px;
  border:none; background:transparent; cursor:pointer; color:#8b7355;
  transition:color .2s; border-bottom:2.5px solid transparent; letter-spacing:.03em;
}
...
.sb {
  font-family:'IBM Plex Mono',monospace; font-size:12px; padding:5px 13px;
  border:1.5px solid #c4a97d; border-radius:20px; background:transparent;
  cursor:pointer; color:#5a4027; transition:background .18s, color .18s;
}
...
.mbtn-cancel {
  ...
  border:1.5px solid #c4a97d; border-radius:8px; background:transparent;
  cursor:pointer; color:#5a4027; transition:background .18s;
}
.mbtn-cancel:hover { background:#e8ddc8; }
.mbtn-add {
  font-family:'IBM Plex Mono',monospace; font-size:13px; padding:10px 22px;
  border:none; border-radius:8px; background:#3d2c1e; color:#f5f0e8;
...
.dinput {
  font-family:'IBM Plex Mono',monospace; font-size:12px; color:#2d1f0f;
  background:#f5f0e8; border:1.5px solid #c4a97d; border-radius:6px;
...
const S = {
  ...
  tabs: { display:"flex", borderBottom:"1.5px solid #d9cdb8", padding:"0 20px" },
  body: { padding:"22px 26px 32px" },
  overlay: {
    position:"fixed", inset:0, background:"rgba(45,31,15,.45)", backdropFilter:"blur(5px)",
    display:"flex", alignItems:"center", justifyContent:"center", zIndex:1000, padding:16,
  },
  modal: {
    background:"#fffdf7", borderRadius:14, padding:28, width:"100%", maxWidth:430,
    boxShadow:"0 24px 60px rgba(0,0,0,.22)", border:"1.5px solid #d9cdb8",
  ...
  banner: {
    background:"linear-gradient(135deg,#fffbf3,#fff5e6)",
    border:"1.5px solid #e8ddc8", borderRadius:10,
